...
module.exports = {
    ...
    validate: function(address){
        const splittedAddress = this.split(address);
        if (!splittedAddress.localPart || !splittedAddress.domainName) return false;
        let localPart = splittedAddress.localPart;
        let domainName = splittedAddress.domainName;
    ...
    split: function(address){
        ...
        if (splittedAddress.length !== 2) return {};
        return {
            localPart: splittedAddress[0],
            domainName: splittedAddress[1]
        };
    },
    /**
